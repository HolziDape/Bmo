...
def _ensure_spotify_running(sp):
    """Startet Spotify falls kein Gerät aktiv ist."""
    try:
        devices = sp.devices()
...
# Wird aufgerufen wenn der Core eine Aktion zurückgibt
# ══════════════════════════════════════════════════════════════════
...
def handle_local_action(action, action_params):
    ...
    return None  # Keine lokale Aktion nötig
...
<div class="app">
  ...
  <div class="quick-btns">
    <button class="qbtn green" onclick="showStats()"><span class="icon">📊</span>Stats</button>
    <button class="qbtn purple" onclick="showSpotify()"><span class="icon">🎵</span>Spotify</button>
    <button class="qbtn orange" onclick="confirmShutdown()"><span class="icon">⏻</span>Shutdown</button>
  </div>
  ...
  <div class="sheet" onclick="event.stopPropagation()">
    ...
    <div style="margin-bottom:20px;">
      ...
      <div style="display:flex;align-items:center;gap:12px;">
        ...
        <input type="range" id="volSlider" min="0" max="100" value="50" style="flex:1;accent-color:var(--green);height:6px;cursor:pointer;" oninput="document.getElementById('volLabel').textContent=this.value+'%'" onchange="setVolume(this.value)">
        <span style="font-size:18px;">🔊</span>
      </div>
...
def _chat_and_act(message):
    """
    Sendet Nachricht an Core (mit remote=True), führt Aktionen lokal aus,
    holt TTS-Audio vom Core.
    Gibt (response_text, audio_b64_oder_None) zurück.
    """
    try:
    ...
    # Lokale Aktion ausführen (Spotify, Shutdown, etc.)
    local_result = handle_local_action(action, action_params)
    if local_result:
        response_text = local_result  # Echtes Ergebnis (z.B. "Ich spiele Coldplay")
